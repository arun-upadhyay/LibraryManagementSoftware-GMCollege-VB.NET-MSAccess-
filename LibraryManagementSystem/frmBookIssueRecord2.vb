...
Public Class frmBookIssueRecord2
    ...
    Public Sub fillAccNO()
        Try
            Dim sql As String = "select Distinct(AccessionNo) from BookIssue_Student"
    ...
    Private Sub txtBookName_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBookName.TextChanged
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT TransactionID as [Transaction ID], IssueDate as [Issue Date], DueDate as [Due Date], Book.AccessionNo as [Accession No], BCode as [Book Code], BookTitle as [Book Title],Author,Subject,ISBN,Edition,Student.StudentID as [Student ID],StudentName as [Student Name],Course,Student.Department, Status from Book,BookIssue_Student,Student where Book.AccessionNo=BookIssue_Student.AccessionNo and BookIssue_Student.StudentID=Student.StudentID and Status='Issued' and Status='Issued' and Issuedate Between #" & dtpDateFrom.Text & "# and #" & dtpDateTo.Text & "# and BookTitle like '" & txtBookName.Text & "%' order by Issuedate desc ", con)
    ...
    Private Sub txtBookName1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBookName1.TextChanged
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT TransactionID as [Transaction ID], IssueDate as [Issue Date], DueDate as [Due Date], Book.AccessionNo as [Accession No], BookIssue_Student.BCode as [Book Code], BookTitle as [Book Title],Author,Subject,ISBN,Edition,Student.StudentID as [Student ID],StudentName as [Student Name],Course,Student.Department, Status from Book,BookIssue_Student,Student where Book.AccessionNo=BookIssue_Student.AccessionNo and BookIssue_Student.StudentID=Student.StudentID and Status='Issued' and DueDate Between #" & DateTimePicker2.Text & "# and #" & DateTimePicker1.Text & "# and BookTitle like '" & txtBookName1.Text & "%' order by DueDate desc ", con)
            Dim myDA As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "BookIssue_Student")
            myDA.Fill(myDataSet, "Book")
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        Try
            ...
            Me.Hide()
            frmBookReturn.Show()
            frmBookReturn.txtTransactionID.Text = dr.Cells("Transaction ID").Value.ToString()
            frmBookReturn.dtpIssueDate.Text = dr.Cells("Issue Date").Value.ToString()
            frmBookReturn.dtpDueDate.Text = dr.Cells("Due Date").Value.ToString()
            frmBookReturn.txtAccessionNo.Text = dr.Cells("Accession No").Value.ToString()
            frmBookReturn.txtBookCode.Text = dr.Cells("Book Code").Value.ToString()
            frmBookReturn.txtBookTitle.Text = dr.Cells("Book Title").Value.ToString()
            frmBookReturn.txtAuthor.Text = dr.Cells("Author").Value.ToString()
            frmBookReturn.txtISBN.Text = dr.Cells("ISBN").Value.ToString()
            frmBookReturn.txtEdition.Text = dr.Cells("Edition").Value.ToString()
            frmBookReturn.txtStudentID.Text = dr.Cells("Student ID").Value.ToString()
            frmBookReturn.txtStudentName.Text = dr.Cells("Student Name").Value.ToString()
            frmBookReturn.txtCourse.Text = dr.Cells("Course").Value.ToString()
            frmBookReturn.txtDepartment.Text = dr.Cells("Department").Value.ToString()
            frmBookReturn.btnSave.Enabled = True
            frmBookReturn.btnDelete.Enabled = False
            frmBookReturn.btnUpdate_record.Enabled = False
    ...
    Private Sub cmbAccNo_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbAccNo.SelectedIndexChanged
        Try
            Dim sql As String = "select Distinct(BCode) from BookIssue_Student where AccessionNo='" & Trim(cmbAccNo.Text) & "'"
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand(sql, con)
            rdr = cmd.ExecuteReader()
    ...
    Private Sub Button3_Click(sender As System.Object, e As System.EventArgs) Handles Button3.Click
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT TransactionID as [Transaction ID], IssueDate as [Issue Date], DueDate as [Due Date], Book.AccessionNo as [Accession No], BookIssue_Student.BCode as [Book Code], BookTitle as [Book Title],Author,Subject,ISBN,Edition,Student.StudentID as [Student ID],StudentName as [Student Name],Course,Student.Department, Status from Book,BookIssue_Student,Student where Book.AccessionNo=BookIssue_Student.AccessionNo and BookIssue_Student.StudentID=Student.StudentID and Status='" & Trim(cmbStatus.Text) & "' and BookIssue_Student.AccessionNo='" & Trim(cmbAccNo.Text) & "'and BookIssue_Student.BCode='" & Trim(cmbBookCode.Text) & "'", con)
            Dim myDA As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "BookIssue_Student")
            myDA.Fill(myDataSet, "Book")
