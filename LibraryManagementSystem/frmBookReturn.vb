...
Public Class frmBookReturn
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            If Len(Trim(txtTransactionID.Text)) = 0 Then
                ...
                Exit Sub
            End If
            If (dtpReturnDate.Value.Date < dtpIssueDate.Value.Date) Then
                MessageBox.Show("Return date can not be less than Issue Date", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                dtpReturnDate.Focus()
                Exit Sub
            End If
    ...
    Public Sub DeleteRecord()
        Try
            ...
            If RowsAffected > 0 Then
                ...
                con = New OleDbConnection(cs)
                con.Open()
                Dim cb2 As String = "Update bookIssue_student set Status = 'Issued' where AccessionNo='" & txtAccessionNo.Text & "'"
                cmd = New OleDbCommand(cb2)
                cmd.Connection = con
                cmd.ExecuteNonQuery()
                con.Close()
    ...
    Public Sub DeleteRecord1()
        Try
            ...
            If RowsAffected > 0 Then
                ...
                con = New OleDbConnection(cs)
                con.Open()
                Dim cb2 As String = "Update bookIssue_Staff set Status = 'Issued' where AccessionNo='" & txtAccessionNo1.Text & "'"
                cmd = New OleDbCommand(cb2)
                cmd.Connection = con
                cmd.ExecuteNonQuery()
                con.Close()
    ...
    Private Sub btnUpdate_record_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate_record.Click
        Try
            ...
            End If
            If (dtpReturnDate.Value.Date < dtpIssueDate.Value.Date) Then
                MessageBox.Show("Return date can not be less than Issue Date date", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                dtpReturnDate.Focus()
                Exit Sub
            End If
            If txtFine.Text = "" Then
