...
Public Class frmBookIssueRecord
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        Try
            ...
            Me.Hide()
            frmBookIssue.Show()
            frmBookIssue.txtTransactionID.Text = dr.Cells("Transaction ID").Value.ToString()
            frmBookIssue.dtpIssueDate.Text = dr.Cells("Issue Date").Value.ToString()
            frmBookIssue.dtpDueDate.Text = dr.Cells("Due Date").Value.ToString()
            frmBookIssue.cmbAccessionNo.Text = dr.Cells("Accession No").Value.ToString()
            frmBookIssue.cmbBookCode.Text = dr.Cells("Book Code").Value.ToString()
            frmBookIssue.TextBox1.Text = dr.Cells("Accession No").Value.ToString()
            frmBookIssue.txtBookTitle.Text = dr.Cells("Book Title").Value.ToString()
            frmBookIssue.txtAuthor.Text = dr.Cells("Author").Value.ToString()
            frmBookIssue.txtCategory.Text = dr.Cells("Subject").Value.ToString()
            frmBookIssue.txtISBN.Text = dr.Cells("ISBN").Value.ToString()
            frmBookIssue.txtEdition.Text = dr.Cells("Edition").Value.ToString()
            frmBookIssue.cmbStudentID.Text = dr.Cells("Student ID").Value.ToString()
            frmBookIssue.txtStudentName.Text = dr.Cells("Student Name").Value.ToString()
            frmBookIssue.txtCourse.Text = dr.Cells("Course").Value.ToString()
            frmBookIssue.txtDepartment.Text = dr.Cells("Department").Value.ToString()
            frmBookIssue.txtStatus.Text = dr.Cells("Status").Value.ToString()
            frmBookIssue.btnUpdate_record.Enabled = True
            frmBookIssue.btnDelete.Enabled = True
            frmBookIssue.btnSave.Enabled = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub Button3_Click(sender As System.Object, e As System.EventArgs) Handles Button3.Click
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT TransactionID as [Transaction ID], IssueDate as [Issue Date], DueDate as [Due Date], Book.AccessionNo as [Accession No],BookIssue_Student.BCode as [Book Code], BookTitle as [Book Title],Author,Subject,ISBN,Edition,Student.StudentID as [Student ID],StudentName as [Student Name],Course,Student.Department, Status from Book,BookIssue_Student,Student where Book.AccessionNo=BookIssue_Student.AccessionNo and BookIssue_Student.StudentID=Student.StudentID and BookIssue_Student.AccessionNo='" & Trim(cmbAccNo.Text) & "' and BookIssue_Student.BCode='" & Trim(cmbBookCode.Text) & "' and BookIssue_Student.Status='" & Trim(cmbStatus.Text) & "' order by IssueDate desc ", con)
            Dim myDA As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "BookIssue_Student")
            myDA.Fill(myDataSet, "Book")
