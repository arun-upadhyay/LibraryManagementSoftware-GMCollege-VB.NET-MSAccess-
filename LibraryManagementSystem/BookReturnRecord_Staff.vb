...
Public Class frmBookReturnRecord_Staff
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        Try
            ...
            Me.Hide()
            frmBookReturn.Show()
            frmBookReturn.txtReturnID1.Text = dr.Cells("Return ID").Value.ToString()
            frmBookReturn.dtpReturnDate1.Text = dr.Cells("Return Date").Value.ToString()
            frmBookReturn.txtFine1.Text = dr.Cells("Fine").Value.ToString()
            ...
            frmBookReturn.TextBox2.Text = dr.Cells("Accession No").Value.ToString()
    ...
    Private Sub Button4_Click(sender As System.Object, e As System.EventArgs) Handles Button4.Click
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT ReturnID as [Return ID],ReturnDate as [Return Date],Fine, BookIssue_Staff.TransactionID as [Transaction ID], IssueDate as [Issue Date], DueDate as [Due Date], Book.AccessionNo as [Accession No],BookIssue_Staff.BCode as [Book Code],BookTitle as [Book Title],Author,Subject,ISBN,Edition,Staff.StaffID as [Staff ID],StaffName as [Staff Name],Staff.Department from Book,BookIssue_Staff,Staff,Return_Staff where Book.AccessionNo=BookIssue_Staff.AccessionNo and BookIssue_Staff.StaffID=Staff.StaffID  and BookIssue_Staff.TransactionID=Return_Staff.TransactionID and AccessionNo='" & cmbAccNo.Text & "' and BookCode='" & cmbBookCode.Text & "' ", con)
            Dim myDA As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "")
            myDA.Fill(myDataSet, "BoBookIssue_Staffok")
            myDA.Fill(myDataSet, "Staff")
            myDA.Fill(myDataSet, "Return_Staff")
            DataGridView1.DataSource = myDataSet.Tables("BookIssue_Staff").DefaultView
            DataGridView1.DataSource = myDataSet.Tables("Book").DefaultView
